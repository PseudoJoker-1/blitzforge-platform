...
3. Пакеты каталога: у ресурсных пакетов проверить `stock_sha256` (после
   ...
   совместимость.
4. Каталог в игре (`examples/catalog`, пакет `blitzforge.catalog`) привязан к
   сборке через `client.builds` и `CLIENT_BUILD` в `main.lua`: обновить оба,
   выпустить новую версию, опубликовать на портале — установленные копии
   предложат обновление сами.
5. Снять объявление.
