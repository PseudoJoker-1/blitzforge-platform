...
(подробно в [LUA_MODS_RU.md](LUA_MODS_RU.md), раздел «Фасады»).
...
[WOTBMOD_PACKAGE_FORMAT_RU.md](WOTBMOD_PACKAGE_FORMAT_RU.md).

Что дальше: [LUA_MODS_RU.md](LUA_MODS_RU.md) — весь Lua API с фасадами;
[API_REFERENCE_RU.md](API_REFERENCE_RU.md) — сгенерированный справочник по
каждому слоту с правами и контекстами; [API_STATUS_RU.md](API_STATUS_RU.md) —
что подтверждено на живом клиенте. Нативный мод (C++) начинается с
`wotbmod new --type native` и [WOTBMOD_PACKAGE_FORMAT_RU.md](WOTBMOD_PACKAGE_FORMAT_RU.md).
